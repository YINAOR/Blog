...
// import BlogPostItemHeaderAuthors from '@site/src/components/Authors'
...
function BlogPostItem(props) {
	...
	const { slug: postId, author, title, image, activityId, oid, bvid } = frontMatter
	...
	const renderPostHeader = () => {
		...
		return (
			<header>
				<TitleHeading
					className={clsx(isBlogPostPage ? 'margin-bottom--md' : 'margin-vert--md', styles.blogPostTitle, isBlogPostPage ? 'text--center' : '')}
				>
			...
			>
				...
				<div className={`col ${isBlogPostPage ? `col--12 article__details` : `col--9`}`}>
					...
					<article className={!isBlogPostPage ? 'margin-bottom--md' : undefined}>
						...
						{/* 标签 */}
						{isBlogPostPage && <div className="text--center margin-bottom--xs padding-bottom--xs">{renderTags()}</div>}
